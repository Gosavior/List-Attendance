...
const FileInvoiceModal = ({ project, onClose, onUpdated, authFetch }) => {
  ...
  const handleSave = async () => {
    if (!editFile) {
      setEditError('Pilih file invoice baru.')
      return
    }
    setEditLoading(true)
    setEditError('')
    try {
      ...
      formData.append('file_invoice', editFile)
      const res = await authFetch(`${API_BASE}/projects/${project.id}/update-invoice`, {
        method: 'PUT',
        body: formData,
      })
      ...
      setEditError(err.message || 'Gagal update Invoice.')
    } finally {
      setEditLoading(false)
    }
  }

  const fileName = project.file_invoice ? project.file_invoice.split('/').pop().split('\\').pop() : null
    ...
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 px-4" onClick={onClose}>
      <div className="w-full max-w-md bg-white rounded-lg overflow-hidden shadow-xl" onClick={e => e.stopPropagation()}>
        <div className="bg-stone-700 px-4 py-3 flex items-center justify-between">
          <h3 className="text-white font-bold">{editMode ? 'Edit Invoice' : 'File Invoice'} — {project.project_name}</h3>
            ...
            project.file_invoice ? (
              <div className="flex flex-col items-center gap-4">
                ...
                <div className="flex gap-2">
                  <a href={`${API_BASE.replace('/api', '')}/${project.file_invoice}`} target="_blank" rel="noopener noreferrer"
              ...
              <div className="flex flex-col items-center gap-3 py-4">
                ...
                <p className="text-gray-500 text-sm">Belum ada file Invoice</p>
              </div>
            )
          ) : (
            <div className="space-y-4">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">File Invoice</label>
...
const AdminInvoice = ({ authFetch }) => {
  ...
  const loadProjects = useCallback(async () => {
    try {
      ...
      const data = await res.json()
      if (data.success) {
        setProjects(data.data.filter(p => p.file_invoice))
      }
    } catch (err) {
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar space-y-3 pr-1">
        {salesWithProjects.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-20 text-gray-400">
            ...
            <p className="text-sm">Belum ada file Invoice.</p>
          </div>
            ...
            >
              <div className="flex items-center gap-3">
                ...
                <div className="text-left">
                  ...
                  <p className="text-gray-500 text-xs">{sales.projects.length} file Invoice</p>
                </div>
              </div>
              <FiChevronDown
                size={18}
          ...
          <table className="w-full text-sm min-w-[600px]">
            <thead>
              <tr className="bg-stone-700 text-left text-white text-xs uppercase tracking-wider">
                ...
                <th className="py-3 px-4 font-semibold text-center">File Invoice</th>
              </tr>
            </thead>
              ...
              {!loading && filtered.length === 0 && (
                <tr>
                  <td colSpan={5} className="py-10 text-center text-gray-400">
                    ...
                    Tidak ada file Invoice ditemukan.
                  </td>
                </tr>
              )}
            </tbody>
...
const Invoice = () => {
  ...
  return isAdmin ? <AdminInvoice authFetch={authFetch} /> : <SalesInvoice authFetch={authFetch} />
}
