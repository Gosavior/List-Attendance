...
const FileQOModal = ({ project, onClose, onUpdated, authFetch }) => {
  const [editMode, setEditMode] = useState(false)
  const [editFile, setEditFile] = useState(null)
  const [editQoNumber, setEditQoNumber] = useState(project.qo_number || '')
  const [editLoading, setEditLoading] = useState(false)
  const [editError, setEditError] = useState('')

  const handleSave = async () => {
    if (!editFile && !editQoNumber.trim()) {
      setEditError('Pilih file baru atau masukkan QO Number.')
      return
    }
    setEditLoading(true)
    setEditError('')
    try {
      ...
      if (editFile) formData.append('file_qo', editFile)
      if (editQoNumber.trim()) formData.append('qo_number', editQoNumber.trim())
      const res = await authFetch(`${API_BASE}/projects/${project.id}/update-qo`, {
        method: 'PUT',
        body: formData,
      })
      ...
      setEditError(err.message || 'Gagal update QO.')
    } finally {
  ...
  const handleFileChange = (e) => {
    ...
    if (file) {
      ...
      setEditQoNumber(nameWithoutExt)
    }
  }

  const fileName = project.file_qo ? project.file_qo.split('/').pop().split('\\').pop() : null
    ...
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 px-4" onClick={onClose}>
      <div className="w-full max-w-md bg-white rounded-lg overflow-hidden shadow-xl" onClick={e => e.stopPropagation()}>
        <div className="bg-stone-700 px-4 py-3 flex items-center justify-between">
          <h3 className="text-white font-bold">{editMode ? 'Edit QO' : 'File QO'} — {project.project_name}</h3>
            ...
            project.file_qo ? (
              <div className="flex flex-col items-center gap-4">
                ...
                <div className="text-center">
                  ...
                  {project.qo_number && <p className="text-xs text-gray-400 mt-1">QO Number: {project.qo_number}</p>}
                  {project.nominal_qo && <p className="text-xs text-gray-400 mt-0.5">Nominal: {formatCurrency(project.nominal_qo)}</p>}
                </div>
                <div className="flex gap-2">
                  <a href={`${API_BASE.replace('/api', '')}/${project.file_qo}`} target="_blank" rel="noopener noreferrer"
                  ...
                  <button onClick={() => { setEditMode(true); setEditFile(null); setEditError(''); setEditQoNumber(project.qo_number || '') }}
              ...
              <div className="flex flex-col items-center gap-3 py-4">
                ...
                <p className="text-gray-500 text-sm">Belum ada file QO</p>
              </div>
            )
          ) : (
            <div className="space-y-4">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">File QO (opsional)</label>
              ...
              <div>
                ...
                <input type="text" value={editQoNumber} onChange={e => setEditQoNumber(e.target.value)}
...
const AdminQO = ({ authFetch }) => {
  ...
  const loadProjects = useCallback(async () => {
    try {
      const res = await authFetch(`${API_BASE}/projects?status=NEAREST,ONGOING,DONE`)
      const data = await res.json()
      if (data.success) {
        setProjects(data.data.filter(p => p.file_qo))
        if (data.salesList) setSalesList(data.salesList)
      }
    } catch (err) {
      console.error('QO load error:', err)
    } finally {
      setLoading(false)
    }
  }, [authFetch])
  ...
  const salesWithProjects = useMemo(() => {
    ...
    const filteredProjects = q
      ? projects.filter(p => {
          const target = `${p.project_name} ${p.customer_name} ${p.sales_name || ''} ${p.qo_number || ''}`.toLowerCase()
    ...
    <div className="w-full h-full flex flex-col">
      ...
      <div className="bg-white rounded-lg shadow p-3 sm:p-4 mb-4 shrink-0">
        <div className="relative w-full sm:w-64">
          ...
          <input value={query} onChange={e => setQuery(e.target.value)} placeholder="Cari project, QO number, sales..."
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar space-y-3 pr-1">
        {salesWithProjects.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-20 text-gray-400">
            ...
            <p className="text-sm">Belum ada file QO.</p>
          </div>
            ...
            >
              <div className="flex items-center gap-3">
                ...
                <div className="text-left">
                  ...
                  <p className="text-gray-500 text-xs">{sales.projects.length} file QO</p>
                </div>
              </div>
              <FiChevronDown
                size={18}
                ...
                <table className="w-full text-sm">
                  <thead>
                    <tr className="bg-stone-100 text-left text-xs uppercase tracking-wider text-stone-500">
                      ...
                      <th className="py-2 px-4 font-semibold">QO Number</th>
                      <th className="py-2 px-4 font-semibold">Nominal</th>
                  ...
                  <tbody className="divide-y divide-gray-50">
                    {sales.projects.map(item => (
                      <tr key={item.id} className="hover:bg-gray-50 transition">
                        ...
                        <td className="py-2.5 px-4 text-gray-700 font-medium text-sm">{item.qo_number || '-'}</td>
...
const SalesQO = ({ authFetch }) => {
  ...
  const filtered = useMemo(() => {
    return projects
      ...
      .filter(p => {
        const target = `${p.project_name} ${p.customer_name} ${p.qo_number || ''}`.toLowerCase()
      ...
      <div className="bg-white rounded-lg shadow p-3 sm:p-4 mb-4 shrink-0">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          <div className="flex flex-wrap gap-1.5">
            {['ALL', 'NEAREST', 'ONGOING', 'DONE'].map(s => (
          ...
          <div className="relative w-full sm:w-64">
            <FiSearch className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400" size={14} />
            <input value={query} onChange={e => setQuery(e.target.value)} placeholder="Cari project, QO number..."
              className="w-full bg-white border border-gray-300 rounded-lg pl-9 pr-3 py-1.5 text-sm text-gray-700 placeholder:text-gray-400 focus:outline-none focus:ring-2 focus:ring-stone-400 focus:border-stone-400" />
          </div>
        </div>
      </div>
          ...
          <table className="w-full text-sm min-w-[600px]">
            <thead>
              <tr className="bg-stone-700 text-left text-white text-xs uppercase tracking-wider">
                ...
                <th className="py-3 px-4 font-semibold">QO Number</th>
                <th className="py-3 px-4 font-semibold">Nominal QO</th>
                <th className="py-3 px-4 font-semibold">Status</th>
                <th className="py-3 px-4 font-semibold text-center">File QO</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-gray-100">
              {loading ? (
                ...
                <tr key={item.id} className="hover:bg-gray-50 transition">
                  ...
                  <td className="py-3 px-4 text-gray-800 font-medium">{item.qo_number || '-'}</td>
...
const QuotationOrder = () => {
  ...
  return isAdmin ? <AdminQO authFetch={authFetch} /> : <SalesQO authFetch={authFetch} />
}
