...
const FileReportModal = ({ project, onClose, onUpdated, authFetch }) => {
  ...
  const handleSave = async () => {
    if (!editFile) {
      setEditError('Pilih file report PDF.')
      return
    }
    setEditLoading(true)
    setEditError('')
    try {
      ...
      formData.append('file_report', editFile)
      const res = await authFetch(`${API_BASE}/projects/${project.id}/update-report`, {
        method: 'PUT',
        body: formData,
      })
      ...
      setEditError(err.message || 'Gagal update Report.')
    } finally {
  ...
  const handleFileChange = (e) => {
    ...
    if (file) {
      ...
      if (ext !== 'pdf') {
        setEditError('Hanya file PDF yang diperbolehkan.')
        setEditFile(null)
        return
      }
      setEditError('')
  ...
  const fileName = project.file_report ? project.file_report.split('/').pop().split('\\').pop() : null
    ...
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 px-4" onClick={onClose}>
      <div className="w-full max-w-md bg-white rounded-lg overflow-hidden shadow-xl" onClick={e => e.stopPropagation()}>
        <div className="bg-stone-700 px-4 py-3 flex items-center justify-between">
          <h3 className="text-white font-bold">{editMode ? 'Edit Report' : 'File Report'} — {project.project_name}</h3>
          ...
          {!editMode ? (
            project.file_report ? (
              <div className="flex flex-col items-center gap-4">
                <div className="w-16 h-16 rounded-lg flex items-center justify-center bg-red-600">
                  <FiFileText size={32} className="text-white" />
                </div>
                <div className="text-center">
                  <p className="font-semibold text-gray-800">{fileName}</p>
                  <p className="text-sm text-gray-500 mt-0.5">PDF</p>
                  {project.nominal_qo && <p className="text-xs text-gray-400 mt-1">Nominal QO: {formatCurrency(project.nominal_qo)}</p>}
                </div>
                <div className="flex gap-2">
                  <a href={`${API_BASE.replace('/api', '')}/${project.file_report}`} target="_blank" rel="noopener noreferrer"
                  ...
                  <button onClick={() => { setEditMode(true); setEditFile(null); setEditError('') }}
              ...
              <div className="flex flex-col items-center gap-3 py-4">
                ...
                <p className="text-gray-500 text-sm">Belum ada file Report</p>
              </div>
            )
          ) : (
            <div className="space-y-4">
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">File Report (PDF)</label>
                <label className="flex items-center gap-2 px-4 py-3 bg-gray-50 border border-gray-300 rounded-lg cursor-pointer hover:bg-gray-100 transition">
                  ...
                  <span className="text-sm text-gray-600 truncate">{editFile ? editFile.name : 'Pilih file PDF...'}</span>
                  <input type="file" accept=".pdf" className="hidden" onChange={handleFileChange} />
                </label>
                <p className="text-xs text-gray-400 mt-1">Hanya file PDF yang diperbolehkan.</p>
              </div>
...
const AdminReport = ({ authFetch }) => {
  ...
  const loadProjects = useCallback(async () => {
    try {
      ...
      const data = await res.json()
      if (data.success) {
        setProjects(data.data.filter(p => p.file_report))
      }
    } catch (err) {
    ...
    <div className="w-full h-full flex flex-col">
      <div className="bg-white rounded-lg shadow p-3 sm:p-4 mb-4 shrink-0">
        <div className="relative w-full sm:w-64">
          ...
          <input value={query} onChange={e => setQuery(e.target.value)} placeholder="Cari project, customer, sales..."
      ...
      <div className="flex-1 overflow-y-auto custom-scrollbar space-y-3 pr-1">
        {salesWithProjects.length === 0 ? (
          <div className="flex flex-col items-center justify-center py-20 text-gray-400">
            ...
            <p className="text-sm">Belum ada file Report.</p>
          </div>
            ...
            >
              <div className="flex items-center gap-3">
                ...
                <div className="text-left">
                  ...
                  <p className="text-gray-500 text-xs">{sales.projects.length} file Report</p>
                </div>
              </div>
              <FiChevronDown
                size={18}
    ...
    <div className='w-full h-full flex flex-col'>
      <div className="bg-white rounded-lg shadow p-3 sm:p-4 mb-4 shrink-0">
        <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="relative w-full sm:w-64">
            <FiSearch className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400" size={14} />
            <input value={query} onChange={e => setQuery(e.target.value)} placeholder="Cari project, customer..."
              className="w-full bg-white border border-gray-300 rounded-lg pl-9 pr-3 py-1.5 text-sm text-gray-700 placeholder:text-gray-400 focus:outline-none focus:ring-2 focus:ring-stone-400 focus:border-stone-400" />
          </div>
        </div>
      </div>
          ...
          <table className="w-full text-sm min-w-[600px]">
            <thead>
              <tr className="bg-stone-700 text-left text-white text-xs uppercase tracking-wider">
                ...
                <th className="py-3 px-4 font-semibold text-center">File Report</th>
              </tr>
            </thead>
              ...
              {!loading && filtered.length === 0 && (
                <tr>
                  <td colSpan={5} className="py-10 text-center text-gray-400">
                    ...
                    Tidak ada file Report ditemukan.
                  </td>
                </tr>
              )}
            </tbody>
...
const Report = () => {
  ...
  return isAdmin ? <AdminReport authFetch={authFetch} /> : <SalesReport authFetch={authFetch} />
}
