...
import headerGms from "../../assets/kop_surat_ASA.png" 
...
const generatePDF = (form, safetyWitness, sig1Data = null, sig2Data = null) => {
  const doc = new jsPDF('portrait', 'mm', 'a4')

  doc.addImage(headerGms, `PNG`, 25, 9, 160, 22, { align: "center" })
  doc.line(10, 33, 200, 33)
  doc.line(10, 32.5, 200, 32.5)

  doc.setFont("times", "bold")
  doc.setFontSize(13)
  doc.text("SERVICE REPORT + SAFETY WITNESS", 105, 42, { align: "center" })
  doc.line(57, 43, 153, 43)
  ...
  doc.rect(swLeft, tblTop, secRight - swLeft, secBottom - tblTop)
  doc.setLineWidth(0.2)

  
  doc.line(colNo, tblTop + headerH, tblRight, tblTop + headerH)
  doc.line(swLeft, tblTop + headerH, secRight, tblTop + headerH)
  ...
  const swCheckX = secRight - 12
  doc.line(swCheckX, tblTop, swCheckX, secBottom)
  ...
  doc.text("Description", swLeft  + 2,  tblTop + 5)
  doc.text("Check",       swCheckX + 1, tblTop + 5)
  ...
  for (let i = 0; i < numRows; i++) {
    const rowTop = tblTop + headerH + i * rowH
    if (i < numRows - 1) {
      doc.line(swLeft, rowTop + rowH, secRight, rowTop + rowH)
    }
    const sw = safetyWitness[i]
    if (sw?.description) {
      doc.text(sw.description, swLeft + 2, rowTop + rowH - 1.5)
    }
    if (sw?.checked) {
      const cx = swCheckX + 6
...
const previewPDF = (form, safetyWitness, sig1Data = null, sig2Data = null) => {
  const doc = generatePDF(form, safetyWitness, sig1Data, sig2Data)
  const pdfBlob = doc.output('blob')
  const url = URL.createObjectURL(pdfBlob)
  window.open(url)
}

const saveReportPDF = (form, safetyWitness, sig1Data = null, sig2Data = null) => {
  const doc = generatePDF(form, safetyWitness, sig1Data, sig2Data)
  doc.save(`service_report_GMS_${Date.now()}.pdf`)
}
...
const ServiceReport = () => {
  ...
  const [safetyWitness, setSafetyWitness] = useState(
    Array.from({ length: 7 }, () => ({ description: '', checked: false }))
  )

  const handleSafetyChange = (index, field, value) => {
    setSafetyWitness(prev => {
      const updated = [...prev]
  ...
  return (
    <div className="w-full min-h-screen bg-gradient-to-b from-slate-50 to-slate-100 pb-24 md:pb-8">
      ...
      { }
      <div className="bg-gradient-to-r from-emerald-700 to-emerald-900 text-white px-4 py-5 md:py-6 shadow-lg sticky top-0 z-20">
        <div className="max-w-4xl mx-auto flex items-center justify-between">
          <div>
            <h1 className="text-lg md:text-xl font-bold tracking-wide">Service Report GMS</h1>
            <p className="text-emerald-200 text-xs md:text-sm">Gandri Mitra Sukses</p>
          </div>
          <button
            onClick={() => navigate('/')}
            className="text-emerald-200 hover:text-white p-2 rounded-lg hover:bg-emerald-800 transition-colors"
          >
      ...
      <div className="max-w-4xl mx-auto px-4 py-5 space-y-4">
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-4">
          ...
          <input name="WR" type="text" value={form.WR} onChange={handleChange} placeholder="Contoh: WR-GMS-250001" className="w-full border border-gray-300 rounded-lg px-4 py-3 text-base focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 outline-none transition-shadow" />
        </div>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-4">
          <div className="flex items-center gap-2 mb-4">
            <div className="w-8 h-8 rounded-lg bg-emerald-100 flex items-center justify-center">
              <svg className="w-4 h-4 text-emerald-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 21V5a2 2 0 00-2-2H7a2 2 0 00-2 2v16m14 0h2m-2 0h-5m-9 0H3m2 0h5M9 7h1m-1 4h1m4-4h1m-1 4h1m-5 10v-5a1 1 0 011-1h2a1 1 0 011 1v5m-4 0h4" /></svg>
            </div>
          ...
          <div className="space-y-3">
            <div>
              ...
              <input name="customerName" value={form.customerName} onChange={handleChange} type="text" placeholder="PT. Nama Perusahaan" className="w-full border border-gray-300 rounded-lg px-4 py-3 text-base focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 outline-none" />
            </div>
            <div>
              <label className="block text-xs font-medium text-gray-500 mb-1">Alamat</label>
              <input name="address" value={form.address} onChange={handleChange} type="text" placeholder="Alamat lengkap" className="w-full border border-gray-300 rounded-lg px-4 py-3 text-base focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 outline-none" />
            </div>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              <div>
                <label className="block text-xs font-medium text-gray-500 mb-1">PIC / Attention</label>
                <input name="pic" value={form.pic} onChange={handleChange} type="text" placeholder="Nama PIC" className="w-full border border-gray-300 rounded-lg px-4 py-3 text-base focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 outline-none" />
              </div>
              <div>
                <label className="block text-xs font-medium text-gray-500 mb-1">No. Telepon</label>
                <input name="phone" value={form.phone} onChange={handleChange} type="tel" placeholder="08xx-xxxx-xxxx" className="w-full border border-gray-300 rounded-lg px-4 py-3 text-base focus:ring-2 focus:ring-emerald-500 focus:border-emerald-500 outline-none" />
              </div>
            </div>
          </div>
        </div>
          ...
          <div className="space-y-3">
            <div className="grid grid-cols-2 gap-3">
              <div>
                ...
                <input name="startDate" value={form.startDate} onChange={handleChange} type="date" className="w-full border border-gray-300 rounded-lg px-3 py-3 text-base focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none" />
              </div>
              <div>
                <label className="block text-xs font-medium text-gray-500 mb-1">Tanggal Selesai</label>
                <input name="endDate" value={form.endDate} onChange={handleChange} type="date" className="w-full border border-gray-300 rounded-lg px-3 py-3 text-base focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none" />
              </div>
            </div>
            <div className="grid grid-cols-2 gap-3">
              <div>
                <label className="block text-xs font-medium text-gray-500 mb-1">Jam Tiba</label>
                <input name="startTime" value={form.startTime} onChange={handleChange} type="time" className="w-full border border-gray-300 rounded-lg px-3 py-3 text-base focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none" />
              </div>
              <div>
                <label className="block text-xs font-medium text-gray-500 mb-1">Jam Selesai</label>
                <input name="endTime" value={form.endTime} onChange={handleChange} type="time" className="w-full border border-gray-300 rounded-lg px-3 py-3 text-base focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none" />
              </div>
            </div>
            <div>
              <label className="block text-xs font-medium text-gray-500 mb-1">Teknisi Leader</label>
              <input name="technician" value={form.technician} onChange={handleChange} type="text" placeholder="Nama teknisi leader" className="w-full border border-gray-300 rounded-lg px-4 py-3 text-base focus:ring-2 focus:ring-blue-500 focus:border-blue-500 outline-none" />
            </div>
          </div>
        </div>
          ...
          <div className="flex flex-wrap gap-2 mb-5">
            {[
              ...
              <label key={item.name} className={`flex items-center gap-2 px-4 py-2.5 rounded-lg border-2 cursor-pointer transition-all text-sm font-medium select-none ${
                item.checked ? 'border-emerald-500 bg-emerald-50 text-emerald-700' : 'border-gray-200 bg-gray-50 text-gray-600 hover:border-gray-300'
              }`}>
                <input type="checkbox" name={item.name} checked={item.checked} onChange={handleChange} className="sr-only" />
                <div className={`w-5 h-5 rounded flex items-center justify-center flex-shrink-0 ${item.checked ? 'bg-emerald-500' : 'bg-white border-2 border-gray-300'}`}>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-4">
          <div className="flex items-center gap-2 mb-4">
            ...
            <h2 className="font-bold text-gray-800">Material & Safety Witness</h2>
          </div>
          ...
          { }
          <h3 className="text-sm font-semibold text-gray-700 mb-2">Safety Witness</h3>
          <div className="overflow-x-auto -mx-4 px-4">
            <table className="w-full min-w-[300px] border-collapse text-sm">
              <thead>
                <tr className="bg-yellow-50">
                  <th className="border border-gray-300 px-2 py-2.5 text-left">Description</th>
                  <th className="border border-gray-300 px-2 py-2.5 text-center w-16">Check</th>
                </tr>
              </thead>
              <tbody>
                {safetyWitness.map((row, i) => (
                  <tr key={i} className="hover:bg-gray-50">
                    <td className="border border-gray-300 px-1 py-1">
                      <input type="text" className="w-full px-2 py-1.5 outline-none rounded text-base" value={row.description} onChange={(e) => handleSafetyChange(i, 'description', e.target.value)} />
                    </td>
                    <td className="border border-gray-300 px-2 py-2 text-center">
                      <input type="checkbox" checked={row.checked} onChange={(e) => handleSafetyChange(i, 'checked', e.target.checked)} className="w-5 h-5 accent-emerald-600" />
                    </td>
                  </tr>
                ))}
              </tbody>
      ...
      <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-gray-200 p-3 md:p-4 shadow-[0_-4px_12px_rgba(0,0,0,0.1)] z-20 md:static md:shadow-none md:border-0 md:bg-transparent">
        <div className="max-w-4xl mx-auto flex gap-2 md:gap-3 md:justify-end">
          <button
            ...
            onClick={() => previewPDF(form, safetyWitness, getSignatureData(sigPad1), getSignatureData(sigPad2))}
          >
            Preview
          </button>
          <button
            ...
            onClick={() => {
              saveReportPDF(form, safetyWitness, getSignatureData(sigPad1), getSignatureData(sigPad2))
              const name = `SR-GMS_${form.customerName || 'Unknown'}_${form.startDate || Date.now()}`
              saveToStorage(name, 'GMS', form)
            }}
          >
            Save PDF
          </button>
